# vbwd-plugin-bot-telegram

> vbwd backend plugin: bot-telegram — Telegram bot adapter

**Type:** Backend plugin · **Host app:** `vbwd-backend` · **Plugin:** `bot-telegram`
...
```bash
git clone https://github.com/VBWD-platform/vbwd-plugin-bot-telegram.git vbwd-backend/plugins/bot-telegram
```

Then register it in `plugins/plugins.json` (`"bot-telegram": { "enabled": true }`)
